angular.module('upi_npc.services', [])

  .service('WordpressCategory', function ($rootScope, $http, $q, WORDPRESS_REST_API_URL) {
  ...
  .service('PostService', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService) {
    ...
    this.createPost = function (post) {
      ...
      AuthService.requestNonce("posts", "create_post")
        .then(function (nonce) {
          var nonce_dfd = $q.defer();
          nonce_dfd.resolve(nonce);
          console.log(nonce);

          $http.jsonp(WORDPRESS_API_URL + "posts/create_post/?callback=JSON_CALLBACK", {
              params: {
                'nonce': nonce,
                'status': post.status,
                'title': post.title,
